...
import { channelApi, ctaPresenteScheduleApi } from '@/lib/api';
...
export default function CtaPresentePage() {
  ...
  const loadChannels = async () => {
    try {
      ...
      setChannels(response.data.filter((c: any) => c.enabled));
    } catch {
      // silent
    } finally {
      setLoading(false);
  ...
  const handleTest = async () => {
    ...
    setTestMessage('Gerando CTA Presente com IA...');
    try {
      const { data } = await ctaPresenteScheduleApi.testNow(selectedChannelId);
      if (data?.success) {
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">CTA Presente</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">
            O CTA Presente e gerado automaticamente usando o prompt do canal
          </p>
        </div>
        <Button
          onClick={handleTest}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs text-amber-300">
              O conteudo do CTA e gerado pelo Grok usando o <strong>Prompt Mestre</strong> e o campo <strong>CTA Presente</strong> configurados em <a href="/dashboard/settings" className="underline">Configuracoes</a>.
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card padding="none" className="overflow-hidden">
        <div className="p-4 border-b border-[var(--border-default)]">
          ...
          <CardDescription>O bot vai postar um CTA Presente automaticamente em cada horario</CardDescription>
        </div>
