...
import { enqueteScheduleApi } from '@/lib/api';
import toast from 'react-hot-toast';

interface EnqueteSchedule {
  id: string;
  time: string;
  enabled: boolean;
}
...
export default function EnqueteSection({ channelId, channelName }: { channelId: string; channelName: string }) {
  ...
  const loadData = async () => {
    try {
      const schedulesRes = await enqueteScheduleApi.getAll(channelId);
      setSchedules(schedulesRes.data);
    } catch (error) {
      console.error('Error loading enquete schedule data:', error);
    }
  };
  ...
  const handleAddSchedule = async () => {
    ...
    try {
      await enqueteScheduleApi.create(newTime, channelId);
